...
from iea_scraper.settings import (LOGGING_EDC_ELEC_DAILY, EDC_MAILING_LIST, EDC_DAILY_JOBS_BATCH_ELEC, MAIL_EDC_SENDER)
from iea_scraper.jobs.utils import scrape_and_report, send_report

import logging

config_logging(LOGGING_EDC_ELEC_DAILY)
...
job_list = list(EDC_DAILY_JOBS_BATCH_ELEC.values())
...
<body>
<h2>IEA-External-DB - EDC Daily Electricity Load Report</h2>
<p>
...
def main():
    ...
    send_report(report=report,
                mail_subject="[IEA-External-DB] EDC Daily Electricity Extractions Report",
