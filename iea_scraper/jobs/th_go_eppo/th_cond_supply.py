...
PRODUCT = "COND"
UNIT = 'KBD'
FILE = 'th_go_eppo_T02_01_02.xls'

URL = 'http://www.eppo.go.th/epposite/images/Energy-Statistics/' \
      'energyinformation/Energy_Statistics/Petroleum/T02_01_02.xls'


class ThCondSupplyJob(ExtDbApiJob):
    """
    This scraper will extract monthly production of condensate data by field
    from the thailand Ministry of Energy webpage
    """
    title: str = "Thailand - Production of Condensate by field"
    ...
    def get_sources(self):
        ...
        source = BaseSource(url=URL,
                            ...
                            path=FILE,
                            long_name=f"{AREA}"
                                      f" Monthly Production of Condensate by field")
    ...
    def __split_df(df: pd.DataFrame) -> pd.DataFrame:
        ...
        It returns a data frame containing production of Condensate by field.
        :param df: data frame containing the whole data from excel sheet.
        :return: df with Condensate production info by field.
        """
    ...
    def __filter_and_concat_monthly_data(self, table_list_2: List[pd.DataFrame]) -> pd.DataFrame:
        ...
        """
        dfs = [self._map_period(df) for df in table_list_2]
        return pd.concat(dfs)

    @staticmethod
    def _map_period(df: pd.DataFrame) -> pd.DataFrame:
        """
        The year is the first row of the period column. Concatenate it to the row values of the period column.
