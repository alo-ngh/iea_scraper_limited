...
class OilInTransitJob(ExtDbApiDedicatedTableJob):
    """
    Oil in Transit - KPLER API
    Provider: KPLER (https://api.kpler.com/)
    """

    title: str = "Kpler - Oil In Transit - Daily Data"
    ...
    source_prefix_data = f'{provider_code.lower()}_transit'

    key_columns = ['provider', 'source', 'Date', 'IMO', 'Product', 'Grade']
    db_schema = 'kpler'
    db_table_prefix = 'oil_in_transit'
    ...
    today = date.today()
    # starting_date: last day of the month, 5 months ago
    starting_date = date(today.year, today.month, 1) + relativedelta(months=-4, days=-1)
    ...
    def get_loaded_vessels_url(self, date_to_process: date) -> str:
        """
        Returns the URL for getting loaded vessels for a given date.
        ...
        params_loaded_vessels = {"metric": "loaded_vessels",
                                 "zones": "world",
        ...
        return f"{endpoint}?{self.get_params(params_loaded_vessels)}"
    ...
    def get_sources(self):
        ...
        starting_date = (self.starting_date if not self.full_load else self.start_history) \
            if not self.period else self.period
        ending_date = self.ending_date if not self.period else self.period

        for date_to_process in reversed(pd.date_range(start=starting_date, end=ending_date, freq='1M')):
            ...
            source_data = BaseSource(code=source_code,
                                     long_name=f"KPLER - loaded_vessels {str_date}",
                                     url=self.get_loaded_vessels_url(date_to_process),
    ...
    def download_source(self, source: BaseSource, http_headers=None):
        """
        Overrides super method to be able to pass the http_header for authentication.
        ...
        logger.debug(f"Waiting {self.delay_between_requests}s before issuing a new HTTP request")
    ...
    def run(self, download=True, parallel_download=False):
        """
        Overriding parent's class to make parallel_download=False as default.
        :param download: bool: If True, download
        :param parallel_download: bool: if True, download in parallel.
        :return: NoReturn
        """
        logger.debug("Assuming parallel_download=False as default to avoid Kpler overload.")
        super().run(download=download, parallel_download=parallel_download)
